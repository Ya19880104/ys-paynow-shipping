...
class YSShippingResponse {
	...
	*/
	...
	public static function handle_callback() {
		...
		// Webhook Token 驗證（防止偽造回調）
		$stored_token = get_option( 'ys_paynow_shipping_webhook_token', '' );
		if ( empty( $stored_token ) ) {
			// 尚未產生 token：自動產生並儲存，同時拒絕本次請求（商家需更新 PayNow 後台的回傳 URL）
			$stored_token = wp_generate_password( 32, false );
			update_option( 'ys_paynow_shipping_webhook_token', $stored_token );
			YSPaynowShipping::log( '[Webhook] Token 尚未設定，已自動產生。請至設定頁複製新的回傳網址並更新 PayNow 後台。', 'warning' );
			wp_die( 'Webhook token not configured', 'YS PayNow Shipping', array( 'response' => 403 ) );
		}
		...
		if ( ! hash_equals( $stored_token, $request_token ) ) {
			YSPaynowShipping::log( '[Webhook] 驗證失敗：Webhook token 不符', 'error' );
			wp_die( 'Invalid webhook token', 'YS PayNow Shipping', array( 'response' => 403 ) );
		}
		...
		// 驗證來源：確認回傳的物流單號與訂單儲存的一致（防止偽造）
		// 只要任一方有值就進行比對；雙方都空時跳過（尚未取號的訂單）
		$stored_logistic_number = $order->get_meta( YSOrderMeta::LogisticNumber );
		if ( ( ! empty( $stored_logistic_number ) || ! empty( $logistic_number ) ) && $stored_logistic_number !== $logistic_number ) {
			YSPaynowShipping::log( sprintf(
				'[Webhook] 驗證失敗：物流單號不符 (訂單 #%d 儲存: %s, 回傳: %s)',
				$order->get_id(),
				$stored_logistic_number,
				$logistic_number
			), 'error' );
			wp_die( 'Logistic number mismatch', 'YS PayNow Shipping', array( 'response' => 403 ) );
		}

		// 驗證來源：確認訂單確實使用 PayNow 物流
		$logistic_service_id = $order->get_meta( YSOrderMeta::LogisticServiceId );
		if ( empty( $logistic_service_id ) ) {
			YSPaynowShipping::log( sprintf( '[Webhook] 驗證失敗：訂單 #%d 非 PayNow 物流訂單', $order->get_id() ), 'error' );
			wp_die( 'Not a PayNow shipping order', 'YS PayNow Shipping', array( 'response' => 403 ) );
		}
