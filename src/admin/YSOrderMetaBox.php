...
class YSOrderMetaBox {
		...
		<div class="ys-paynow-meta-box">
			...
			<?php if ( ! empty( $logistic_number ) ) : ?>
				<div class="ys-paynow-actions-grid">
					...
					<a href="#" class="button button-primary ys-paynow-print-btn" data-url="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-ajax.php?action=ys_paynow_print_label&order_ids=' . $order->get_id() . '&service=' . $logistic_service_id ), 'ys-paynow-print-label' ) ); ?>">
	...
	public static function ajax_change_store() {
		...
		if ( empty( $store_id ) || empty( $store_name ) ) {
			wp_send_json_error( __( '門市資料不完整，請重新選擇', 'ys-paynow-shipping' ) );
		}
