...
class YSStoreSelector {
	...
	public static function ajax_get_map_url() {
		...
		if ( $is_admin_context ) {
			...
			$return_url = esc_url_raw( add_query_arg( array(
				'cid'              => WC()->cart ? WC()->cart->get_cart_hash() : '',
				'logistic_service' => $logistic_service,
				'method_id'        => $method_id,
				'product'          => '40',
	...
	public static function handle_cvs_callback() {
		// ★ 基本驗證：檢查 cart hash 是否匹配（防止偽造 callback）
		$cid = isset( $_GET['cid'] ) ? sanitize_text_field( wp_unslash( $_GET['cid'] ) ) : '';
		if ( ! empty( $cid ) && WC()->cart && $cid !== WC()->cart->get_cart_hash() ) {
			YSPaynowShipping::log( '[CVS Callback] Cart hash mismatch: expected=' . WC()->cart->get_cart_hash() . ', got=' . $cid, 'warning' );
		}
